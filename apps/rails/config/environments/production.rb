...
Rails.application.configure do # rubocop:todo Metrics/BlockLength
  ...
  # config.active_job.queue_name_prefix = "flatirons_architecture_production"
  ...
  config.hosts << (ENV['HOST_URL'] || '').gsub('https://', '')

  config.hosts << ENV['ALLOWED_HOSTS'] if ENV['ALLOWED_HOSTS']

  config.hosts << IPAddr.new(ENV['ALLOWED_IP']) if ENV['ALLOWED_IP']
end
